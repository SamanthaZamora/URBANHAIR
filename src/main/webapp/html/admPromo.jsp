...
<body>
  ...
  <section class="home-section">
    ...
    <!-- TITULO CON IMG-->
    <div class="title">
      <img src="img/promo.png" width="70px" height="70px">
      <div class="titulo">
      ...
      <div class="boton boton-mas">
         <a type="button" class="btn btn-lg" href="<%=request.getContextPath()%>/ServletPromo?action=new">+</a>
      </div>
    </div>
        ...
        <thead class="table-secondary">
          <tr>
            ...
            <th class="enc" scope="col">Descripción</th>
             <th class="enc" scope="col">Imagen</th>
            <th class="enc" scope="col">Fecha Inicio</th>
            <th class="enc" scope="col">Fecha Fin</th>
            <th class="enc" scope="col">Precio</th>
            <th class="enc" scope="col">Acciones</th>
          </tr>
        </thead>
        <tbody>
        
             <c:forEach var="promo" items="${listaPromo}">
				<tr>
					<th class="enc" scope="row"><c:out value="${promo.idPromo}" /></th>
					<td><c:out value="${promo.nombre}" /></td>
					<td><c:out value="${promo.descripcion}" /></td>
					<td><c:out value="${promo.imgURL}" /></td>
					<td><c:out value="${promo.fechaInicio}" /></td>
					<td><c:out value="${promo.fechaFin}" /></td>
					<td><c:out value="${promo.precio}" /></td>					
					<td>
						<a href="<%=request.getContextPath()%>/ServletPromo?action=edit&idPromo=${promo.idPromo}"><i class="fas fa-edit"></i></a>
						&nbsp;&nbsp;&nbsp;&nbsp; 
						<a href="ServletPromo?action=delete&idPromo=${promo.idPromo}"><i class="fas fa-trash-alt"></i></a>
					</td>
				</tr>
			</c:forEach>
        </tbody>
